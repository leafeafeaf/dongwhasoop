...
function Intro() {
  ...
  const { title, cover } = location.state || { title: "", cover: "" };
    ...
    >
      ...
      <div className="flex flex-col lg:flex-row justify-center items-center h-full gap-[5vh] mt-[5vh] lg:mt-[4vh]">
        ...
        <div className="flex flex-col items-center">
          ...
          />
          <h2 className="mt-4 text-[4vh] sm:text-[5vh] font-bazzi text-center text-[#4e4e4e] text-outline">{title}</h2>
        </div>

        {/* 버튼 영역 */}
        <div className="flex flex-col gap-[4vh] mt-[4vh] lg:mt-0">
          <button onClick={() => navigate("/introsong")}>
            <img src={Song} alt="동요" className="w-[45vw] sm:w-[35vw] lg:w-[18vw] max-w-[400px]" />
          </button>
          <button onClick={() => navigate("/introbook")}>
